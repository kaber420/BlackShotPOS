                        ...
                        <div class="flex flex-col p-3 rounded-xl border {
                            ...
                            <div class="flex items-start gap-2">
                                ...
                                <div class="flex flex-col items-end gap-1 shrink-0">
                                    ...
                                    <!-- Acciones Unificadas -->
                                    {#if !isFinished && (view === 'kitchen' || (view === 'orders' && canManageKitchenStatus))}
                                        <div class="flex gap-1.5 mt-1">
                                            ...
                                            {:else if item.status === 'READY' && view === 'orders'}
                                                <Button variant="success" size="xs" onclick={() => onItemComplete?.(order, item)}>
                                                    Entregar
                                                </Button>
                                            {/if}
                                        </div>
        ...
        {:else}
            ...
            <div class="card-actions flex gap-2 mt-auto pt-4 border-t border-base-200">
                ...
                <Button variant={order.status === 'PENDING' ? 'secondary' : 'primary'} class="flex-1 text-base md:text-lg font-black" onclick={() => onCompleteOrder?.(order)}>
                    {order.status === 'PENDING' ? 'Empezar Orden' : '✓ Orden Lista'}
                </Button>
            </div>
        {/if}
    </div>
